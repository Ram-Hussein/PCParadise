...
<div class="container py-5">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="country" name="country" class="form-select">
                                    <option value="{{ Auth::user()->country_id }}" selected>{{ Auth::user()->country->name }}</option>
                    ...
                    <div class="accordion" id="ordersAccordion">
                        ...
                        <div class="card mb-3 border-0 shadow-sm" style="border-radius: 12px; overflow: hidden;">
                            <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center entry-card-header" 
                                 data-bs-toggle="collapse" data-bs-target="#collapseOrder{{ $order->id }}">
                                    ...
                                    <div class="ms-4 d-none d-md-block">
                                        ...
                                        <span class="badge
                                         @switch($order->status->status)
                                             ...
                                             @break                                                 
                                         @endswitch
                                         status-badge">{{ $order->status->status }}</span>
                                    </div>
                                </div>
                                <div class="text-end">
                                    <span class="text-muted small d-block">Order Total</span>
                                    <span class="fw-bold" style="color: var(--navy-text);">${{$order->Total}}</span>
                                </div>
                            </div>
                            
                            <div id="collapseOrder{{ $order->id }}" class="collapse" data-bs-parent="#ordersAccordion">
                                <div class="card-body pt-0 border-top">
                                    <div class="mt-3">
                                        <p class="mb-2 small"><strong style="color: var(--primary-purple);">Description:</strong> {{ $order->description }}</p>
                                        <div class="entry-details-area">
                                            @foreach ($order->items as $item)
                                                <div class="d-flex justify-content-between small">
                                                <a href="/Product/{{ $item->product_id }}" class="text-decoration-none fw-bold"><span>{{$item->product->name}}</span></a>
                                        ...
                                        @if (($order->status->status != 'Cancelled') and ($order->status->status != 'Delivered'))
                                            ...
                                            <button class="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target="#cancelOrderModal{{ $order->id }}">Cancel Order</button>
                                        </div>
                                        <!-- Cancel Order Modal -->
  ...
  <input class="form-check-input" type="checkbox" value="1" name="admin" id="checkDefault">
  <label class="form-check-label" for="checkDefault">
    Default checkbox
                    ...
                    <div class="accordion" id="addressAccordion">
                        ...
                        <div class="card mb-3 border-0 shadow-sm" style="border-radius: 12px; overflow: hidden;">
                            <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center entry-card-header" 
                                 data-bs-toggle="collapse" data-bs-target="#collapseAddr{{ $address->id }}">
                                <div class="d-flex align-items-center">
                                    <i class="fas fa-chevron-down me-3 text-muted small"></i>
                                    <div>
                                ...
                                <div class="text-end">
                                    ...
                                    <span class="fw-bold" style="color: var(--navy-text);">{{ $address->City }}, {{$address->State}}</span>
                                </div>
                            </div>
                            
                            <div id="collapseAddr{{ $address->id }}" class="collapse" data-bs-parent="#addressAccordion">
                                <div class="card-body pt-0 border-top">
                                    <div class="mt-3">
                                        <div class="entry-details-area">
                                            <p class="mb-1 small"><strong>{{ Auth::user()->fname }} {{ Auth::user()->lname }}</strong></p>
                                            <p class="mb-1 small"><strong>{{Auth::user()->email}} | 0{{Auth::user()->PhoneNumber}}</strong></p>
                                            <p class="mb-1 small">{{$address->StreetAddress}}</p>
                                            <p class="mb-0 small">{{ $address->City }}, {{ $address->State }} | {{ $address->PostalCode }}</p>
                                        </div>

                                        <div class="d-flex justify-content-end mt-3 gap-2">
                                            <button class="btn btn-outline-secondary btn-sm" data-bs-toggle="collapse" data-bs-target="#editAddr{{ $address->id }}">Edit</button>
                                            <button class="btn btn-outline-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteAddressModal{{ $address->id }}">Delete</button>
                                        </div>

                                        <!-- Nested Edit Form -->
                                        <div class="collapse edit-form-container" id="editAddr{{ $address->id }}">
...
{{-- <script>
        ...
        function cancelOrder(orderId) {
            const confirmed = confirm(`Are you sure you want to cancel order ${orderId}? This action is irreversible.`);
            if (confirmed) {
